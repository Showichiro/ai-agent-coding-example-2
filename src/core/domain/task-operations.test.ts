...
import { filterTasksByStatus, sortTasksByCreatedDate, sortTasksByDueDate, SortOrder } from './task-operations';

describe('Task Operations', () => {
  const createTestTasks = (): Task[] => {
    const task1 = createTask({ title: 'Task 1' });
    const task2 = createTask({ title: 'Task 2', dueDate: new Date('2024-12-31') });
    const task3 = createTask({ title: 'Task 3', dueDate: new Date('2024-01-15') });
  ...
  describe('sortTasksByCreatedDate', () => {
    ...
    it('should sort tasks by created date descending', () => {
      ...
      expect(sorted[1].createdAt.getTime()).toBeGreaterThanOrEqual(sorted[2].createdAt.getTime());
    });
  });
