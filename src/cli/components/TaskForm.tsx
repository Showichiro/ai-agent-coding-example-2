...
export function TaskForm({ mode, task, onSubmit, onCancel }: TaskFormProps) {
  ...
  const [currentField, setCurrentField] = useState<'title' | 'description' | 'dueDate'>('title');
  ...
  return (
    <Box flexDirection="column" padding={1} borderStyle="single">
      ...
      <Box paddingTop={1}>
        <Box width={15}>
          <Text color={currentField === 'title' ? 'blue' : undefined}>Title: </Text>
        </Box>
        <Box flexGrow={1}>
          <Text color={inputMode && currentField === 'title' ? 'green' : undefined}>
            {title || '[Enter title]'}
      ...
      <Box>
        <Box width={15}>
          <Text color={currentField === 'description' ? 'blue' : undefined}>Description: </Text>
        </Box>
        <Box flexGrow={1}>
          <Text color={inputMode && currentField === 'description' ? 'green' : undefined}>
      ...
      <Box>
        <Box width={15}>
          <Text color={currentField === 'dueDate' ? 'blue' : undefined}>Due Date: </Text>
        </Box>
        <Box flexGrow={1}>
          <Text color={inputMode && currentField === 'dueDate' ? 'green' : undefined}>
            {dueDate || '[YYYY-MM-DD]'}
