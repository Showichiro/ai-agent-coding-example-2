...
export function FilterControls({ currentFilter, currentSort, currentOrder }: FilterControlsProps) {
  return (
    <Box flexDirection="column" paddingY={1}>
      <Box>
        <Text>Filter: </Text>
        <Text color={currentFilter === 'all' ? 'blue' : undefined}>{getFilterDisplay('all')} </Text>
        <Text color={currentFilter === 'todo' ? 'blue' : undefined}>{getFilterDisplay('todo')} </Text>
        <Text color={currentFilter === 'in_progress' ? 'blue' : undefined}>{getFilterDisplay('in_progress')} </Text>
        <Text color={currentFilter === 'done' ? 'blue' : undefined}>{getFilterDisplay('done')}</Text>
      </Box>
      ...
      <Box>
        ...
        <Text color={currentSort === 'dueDate' ? 'blue' : undefined}>
          {currentSort === 'dueDate' ? getSortDisplay(currentSort, currentOrder) : '[Due Date]'}
        </Text>
      </Box>
    </Box>
  );
